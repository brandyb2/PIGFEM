...
class DenseMatrix
{
	public:
		...
		*/
		...
		// These functions perform normal matrix multiplication with the argument
		inline
		DenseMatrix<T> operator* (const DenseMatrix<T>& rhs) const
		{
		...
		// These functions perform some sort of incrementing of the matrix
		inline
		...
		// These functions perform right hand side multiplication of the matrix
		template <class U>
		friend std::vector<U> operator* (std::vector<U> lhs, DenseMatrix<U>& rhs);

		template <class U>
		friend DenseMatrix<U> operator* (U lhs, DenseMatrix<U>& rhs);
...
template <typename T>
inline
void DenseMatrix<T>::reshape(size_t i, size_t j)
{
	if (i*j != _m*_n)
		err_message("Invalid matrix reshape size");
...
template <class T>
inline
std::vector<T> operator* (std::vector<T> lhs, DenseMatrix<T>& rhs)
{
...
template <class T>
inline
DenseMatrix<T> operator* (T lhs, DenseMatrix<T>& rhs)
{
...
template<>
inline
std::vector<double> operator* (std::vector<double> lhs, DenseMatrix<double>& rhs)
{
	std::vector<double> ret(rhs.n_cols());
	cblas_dgemv(CblasRowMajor, CblasTrans, rhs.n_rows(), rhs.n_cols(), 1.0, rhs.data(), rhs.n_cols(), lhs.data(), 1, 0.0, ret.data(), 1);
	return ret;
}

template<>
inline
std::vector<float> operator* (std::vector<float> lhs, DenseMatrix<float>& rhs)
{
	std::vector<float> ret(rhs.n_cols());
	cblas_sgemv(CblasRowMajor, CblasTrans, rhs.n_rows(), rhs.n_cols(), 1.0, rhs.data(), rhs.n_cols(), lhs.data(), 1, 0.0, ret.data(), 1);
	return ret;
}
