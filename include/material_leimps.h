...
##################################################################################
*/
#ifndef _LEIMPSMATERIAL_H_
#define _LEIMPSMATERIAL_H_
...
class LinearElasticIsotropicMaxPrincipalStressMaterial : public Material
{
	protected:
		double _E;
		double _nu;
	...
	public:
		LinearElasticIsotropicMaxPrincipalStressMaterial();
		virtual ~LinearElasticIsotropicMaxPrincipalStressMaterial() {};

		// Main constitutive subroutine
		virtual Material::output_params* Constitutive(Material::input_params& input);

		// Helper functions
		virtual material_type get_type() {return LINEAR_ELASTIC_ISOTROPIC_MAX_PRINCIPAL_STRESS;};
