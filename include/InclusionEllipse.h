...
##################################################################################
*/
#ifndef _ELLIPSE_INCLUSION_H_
#define _ELLIPSE_INCLUSION_H_


#include "Inclusion.h"

class Ellipse_Inclusion : public Inclusion
{
	private:
		std::vector<double> _center;
		double _alpha, _ca, _sa; // angle of major axis wrt global coordinates
		double _a;
		double _b;

	public:
		Ellipse_Inclusion();
		Ellipse_Inclusion(Ellipse_Inclusion & other_inc);
		virtual ~Ellipse_Inclusion();
		...
		virtual inclusion_type get_type() {return ELLIPSE;};
