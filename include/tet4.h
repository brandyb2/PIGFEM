...
class Tet4 : public Elem
{
	...
	private:
		...
		void decomposePrisms(const std::vector<std::vector<short_id_type> >& old_int, const std::vector<std::vector<short_id_type> >& old_coh, const std::vector<Material*>& old_mat,
							 std::vector<std::vector<short_id_type> >& new_int, std::vector<std::vector<short_id_type> >& new_coh, std::vector<Material*>& new_mat);
		// helper function for performing the decomposition of prisms into tets
		std::vector<std::vector<short_id_type> > partitionPrism(std::vector<short_id_type> local_ids, std::vector<id_type> global_ids,
																bool bottom_known = true);
};
