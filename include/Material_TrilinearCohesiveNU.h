...
##################################################################################
*/
#ifndef _NU_TL_AUG_COH_MATERIAL_H_
#define _NU_TL_AUG_COH_MATERIAL_H_
 ...
 */
class TrilinearCohesiveNUMaterial : public Material
{
	protected:
		// Linear elastic parameters
		double _sigma_c;		// Maximum traction
		double _delta_c1;		// opening at peak traction
		double _delta_c2;		// Opening at the end of the plateau
		double _delta_c3;		// Opening at fully failed
	...
	public:
		TrilinearCohesiveNUMaterial();
		virtual ~TrilinearCohesiveNUMaterial() {};

		// Main constitutive subroutine
		virtual Material::output_params* Constitutive(Material::input_params& input);

		// Helper functions
		virtual material_type get_type() {return TL_COHESIVE_NO_UNLOADING;};
