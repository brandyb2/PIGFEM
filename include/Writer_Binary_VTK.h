...
##################################################################################
*/
#ifndef _Writer_Binary_VTK_H_
#define _Writer_Binary_VTK_H_
 ...
 */
class Writer_Binary_VTK : public Writer_VTK
{
	public:
		 ...
		 */
		Writer_Binary_VTK(Problem* prob);
		virtual ~Writer_Binary_VTK();
		Writer_Binary_VTK(const Writer_Binary_VTK& other);
		bool _isBigEndian;
	...
	private:
		...
		/*
		 * Writes an ascii string to the output file
		 */
		void writeASCII(std::ofstream& myfile, std::string out);

		/*
		 *
		 */
		template<class T>
		void writeBinary(std::ofstream& myfile, const T* data, id_type nData);
 ...
 */
inline
void Writer_Binary_VTK::writeASCII(std::ofstream& myfile, std::string out)
{
	out = out + "\n";
	myfile.write(out.c_str(), out.length());
}
...
template<class T>
inline
void Writer_Binary_VTK::writeBinary(std::ofstream& myfile, const T* data, id_type nData)
{
	if (_isBigEndian)
	{
		myfile.write((const char *)(data), sizeof(T)*nData);
	}
	else
	{
		std::vector<T> swapData(nData);
		for (id_type i=0; i<nData; ++i )
		{
			swapData[i] = Utilities::byteSwap(data[i]);
		}
		myfile.write((const char *)(swapData.data()), sizeof(T)*nData);
	}
}
