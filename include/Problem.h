...
class Problem
{
	...
	public:
		...
		void add_body_load(BodyLoad* load) {_body_loads.push_back(load);};
		...
		Writer* getHomogenizationWriter() const {return _homogenization_writer;};
		...
		virtual myFunctor<std::vector<double> >* getStrainKernel();
		virtual myFunctor<std::vector<double> >* getStressKernel();
	...
	protected:
		...
		// FROM HERE ON OUT IS INFORMATION AND METHODS ABOUT THE MULTISCALE/RVE MODEL
		// =====================================================================================

		// the load that's fed down from the macroscale (strain, heat flux, etc)
		std::vector<double> _macroscaleValue;

		// The most recent value for the homogenized parameter
		std::vector<double> _homogneizedValue;

	public:

		// Function used to homgenize the quantity of interest
		virtual void homogenize();

		virtual std::vector<double> computeMacroscaleSolutionContribution(const std::vector<double>& coords);

		std::vector<double> getMacroscaleValue()
		{
			if (subscale())
				return _macroscaleValue;
			else
				err_message("Attempting to get a macroscopic value of a non-subscale problem");
		};
		void setMacroscaleValue(std::vector<double> val)
		{
			_macroscaleValue = val;
			_subscale = true;
		};

		std::vector<double> getHomogenizedValue() {return _homogneizedValue;};
