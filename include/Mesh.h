...
class Mesh /* inherit from charm object here??? */
{
	...
	public:
		...
		*/
		...
		const std::vector<double>& origin() const;
	...
	private:
		...
		std::vector<double> _enrichment_edge_dist;
		...
		std::vector<std::vector<id_type> > _enrich_node_to_edge;
	...
	public:
		...
		std::vector<id_type> get_enriched_edge_local(id_type idx);
		id_type get_enriched_inclusion_local(id_type idx);
		double getEnrichementScaleFactorLocal(id_type l_node);
		double getEnrichementScaleFactorGlobal(id_type l_node);

		// Function to compute how far along an edge the given enrichment node is
		double getEnrichmentEdgeDistance(id_type id);
		...
		std::vector<id_type> get_edge_from_enrich_node(id_type enrich_id);
	...
	private:
		...
		void add_enrich_node_to_elem(std::vector<id_type> edge, id_type inclusion, id_type n, id_type n_initial);
