...
##################################################################################
*/
#ifndef _Writer_VTK_H_
#define _Writer_VTK_H_
...
class Writer_VTK : public Writer
{
	public:
		 ...
		 */
		Writer_VTK(Problem* prob);
		Writer_VTK();
		virtual ~Writer_VTK();
		Writer_VTK(const Writer_VTK& other);
	...
	protected:
		...
		/*
		 * Some booleans on some options to actually print some of the data or not
		 */
		bool _printRHS;
		bool _printPartition;
		bool _printStrain;
		bool _printStress;
		...
		virtual void write_nodal_data(std::ofstream& myfile,
									  ...
									  const std::vector<std::vector<float> >& remote_sol_lists, const std::vector<std::vector<float> >& remote_rhs_lists) = 0;
