...
Material* LinearElasticIsotropicMaxPrincipalStressMaterial::allocate()
{
	return new LinearElasticIsotropicMaxPrincipalStressMaterial;
}
// This function assumes the current material is set up correctly. Which it should't be possible for it not to be
void LinearElasticIsotropicMaxPrincipalStressMaterial::copy(Material* other_mat)
{
	...
	other_mat->set_parameter("sigma_max", _sigma_max);
}


LinearElasticIsotropicMaxPrincipalStressMaterial::LinearElasticIsotropicMaxPrincipalStressMaterial()
	: _sigma_max(max_max_p_stress), n_set(0), E_set(false), nu_set(false), lambda_set(false), mu_set(false), K_set(false),
	  _thermal_exp(0.0), _thermal_exp_set(false)
{
}

std::vector<std::string> LinearElasticIsotropicMaxPrincipalStressMaterial::internal_vars_name()
{
	std::vector<std::string> vec = {"damage"};
	return vec;
}

std::vector<bool> LinearElasticIsotropicMaxPrincipalStressMaterial::internal_vars_print()
{
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::computeLinearDmat(Material::input_params& input)
{
	if(n_set<2)
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::apply_damage(DenseMatrix<double>& D, double damage)
{
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::compute_stress(std::vector<double>& stress, const DenseMatrix<double>& D, const std::vector<double>& strain)
{
	unsigned char nrows = D.n_rows();
...
Material::output_params* LinearElasticIsotropicMaxPrincipalStressMaterial::Constitutive(Material::input_params& input)
{
	double damaged = 0.999999;
	...
	// If the material is still undamaged, I need to check the stresses and possibly recompute the matrix and stress
	// Also only need to check if the max principle stress has actually been changed from its default really high value
	//	This is in case we want to set the max principal stress mid-simulation based on some macroscopic criterion
	//	Checking it here just saves some computations before we reach that point
	if (curr_damage == 0.0 && _sigma_max != max_max_p_stress)
	{
		compute_stress(stress, _linear_Dmat, strain);
		std::vector<double> principal_stresses = Utilities::principal_stress_strain(stress);
		if (principal_stresses[0] >= _sigma_max)
		{
			// Update the damage
			curr_damage = damaged;
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_parameter(std::string name, double val)
{
...
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_parameter(std::string name)
{
	std::transform(name.begin(), name.end(), name.begin(), ::toupper); // Capitilize the name
	if(name=="E" || name=="YOUNG'S MODULUS" || name=="YOUNGS MODULUS")
		return get_E();
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_E(double E)
{
	if(n_set>=2 && !E_set)
...
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_E()
{
	if(n_set>=2 || E_set)
		return _E;
	else
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_nu(double nu)
{
	if(n_set>=2 && !nu_set)
		err_message("Can only set 2 independant parameters in a linear elastic isotropic material.");
...
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_nu()
{
	if(n_set>=2 || nu_set)
		return _nu;
	else
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_lambda(double lambda)
{
...
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_lambda()
{
	if(n_set>=2 || lambda_set)
		return _lambda;
	else
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_mu(double mu)
{
	if(n_set>=2 && !mu_set)
		err_message("Can only set 2 independant parameters in a linear elastic isotropic material.");
...
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_mu()
{
	if(n_set>=2 || mu_set)
		return _mu;
	else
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_K(double K)
{
	if(n_set>=2 && !K_set)
		err_message("Can only set 2 independant parameters in a linear elastic isotropic material.");
...
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_K()
{
	if(n_set>=2 || K_set)
		return _K;
	else
...
void LinearElasticIsotropicMaxPrincipalStressMaterial::set_sigma_max(double sm)
{
	_sigma_max = sm;
}
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_sigma_max()
{
	return _sigma_max;
}


void LinearElasticIsotropicMaxPrincipalStressMaterial::set_thermal_exp(double val)
{
	_thermal_exp = val;
	_thermal_exp_set = true;
}
double LinearElasticIsotropicMaxPrincipalStressMaterial::get_thermal_exp()
{
	if (_thermal_exp_set)
		return _thermal_exp;
	else
