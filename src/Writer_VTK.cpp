...
Writer_VTK::Writer_VTK(Problem* prob)
	: Writer(prob), _printRHS(false), _printPartition(false), _printStrain(true), _printStress(true),
	  _output_cohesive(false)
{}
			...
			for (int part=1; part<nproc; ++part)
			{
				...
				if(_prob->get_classification() == STRUCTURAL)
				{
					...
					if (_printStress)
					{
						...
						id_type nelem = remote_mats[part-1].size();
						int stride = stresses.size() / nelem;
									 ...
									 std::vector<std::vector<float> >& avg_strain, std::vector<std::vector<float> >& avg_stress)
{
	...
	// Functors for use in element averaging
	VolumeFunctor volume;
	myFunctor<std::vector<double> > *stressFunctor = NULL;
	myFunctor<std::vector<double> > *strainFunctor = NULL;
	if (_prob->get_classification() == STRUCTURAL && _printStrain)
	{
		strainFunctor = _prob->getStrainKernel();
		strainFunctor->_ptr = (void*) _prob;
	}
	if (_prob->get_classification() == STRUCTURAL && _printStress)
	{
		stressFunctor = _prob->getStressKernel();
	...
	{
		Elem* el = (*it);
		id_type id = el->get_id();
		id_type l_elem = mesh->global_to_local_elem(id);
		...
		for(id_type ie=0; ie<el->n_integration_elem(); ++ie)
		{
			...
			Material* mat = mesh->get_element_material_global(id, ie);
			for (auto it2=print_internal_vars_to_init.begin(), end2=print_internal_vars_to_init.end(); it2!=end2; ++it2)
			{
				...
				if (it2->first == "damage")
					if (mesh->get_element_material_global(id, ie)->get_name() != "Epoxy")	// FIXME: I really don't like this hardcoding
			...
			for (id_type iv=0; iv<names.size(); ++iv)
			{
				if (print[iv])
				{
					...
					for (id_type qp=0; qp<nqp; ++qp)
						internal_vars_to_avgs[names[iv]][curr_elem] += _prob->get_internal_vars()->get_internal_var_global(id, initial_qp+qp, iv);
					internal_vars_to_avgs[names[iv]][curr_elem] /= nqp;
				}
			}

			// Compute the averages of the strains and stresses
			if (_prob->get_classification() == STRUCTURAL && (_printStrain || _printStress))
			{
				// Compute the volume of the current integration domain
				Integrator<double> volIntegrator;
				float totalVolume = Utilities::dtos(volIntegrator.integrateElemSimple(_prob, el, &volume, ie));

				Integrator<std::vector<double> > SSIntegrator;
				if (_printStrain)
				{
					avg_strain[curr_elem] = Utilities::dtos(SSIntegrator.integrateElem(_prob, el, strainFunctor, ie));
					avg_strain[curr_elem] = Utilities::scale(avg_strain[curr_elem], 1./totalVolume);
				}
				if (_printStress)
				{
					avg_stress[curr_elem] = Utilities::dtos(SSIntegrator.integrateElem(_prob, el, stressFunctor, ie));
					avg_stress[curr_elem] = Utilities::scale(avg_stress[curr_elem], 1./totalVolume);
				}
			}
		...
		if (_output_cohesive && mesh->is_cohesive())
		{
			for (id_type ce=0; ce<(*it)->n_cohesive_elem(); ++ce)
			{
				...
				if (_prob->get_classification() == STRUCTURAL)
				{
					id_type voigt = avg_stress[0].size();
					if (_printStrain)
	...
	if (strainFunctor != NULL)
		delete strainFunctor;
	if (stressFunctor != NULL)
