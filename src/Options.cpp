...
Options::Options()
{
	...
	_string_options["-HOMOGENIZATION_FILE"] = "homogenized.csv";
	...
	_bool_options["-HOMOGENIZE"] = false;
...
void Options::parseInput(int argc, char* argv[])
{
	...
	for (int i=1; i<argc; i+=2)
	{
		...
		if (flag=="-OUT_FOLDER")
		{
			...
			setStatsFile(val);
		else if (flag=="-HOMOGENIZATION_FILE")
			setHomoFile(val);
		else if (flag=="-MESH")
			setMeshFile(val);
		else if (flag=="-LOG")
...
bool Options::optionSet(std::string flag)
{
...
{
	// Read the format of the boolean
	...
	// Set the flag
	_bool_options[flag] = res;

	// Set the global variable so it's accessible by the PIGFEMPrint function
	if (flag=="-WRITE_TO_SCREEN")
		writeToScreen = res;

	// Should homogenize all subscale models
	if (flag == "-SUBSCALE")
		if (res)
			_bool_options["-HOMOGENIZE"] = true;
}
...
void Options::setHomoFile(std::string homo_file)
{
	// Check if the the end of the filefile is ".csv"
	if (homo_file.find(".csv", homo_file.length()-4) == std::string::npos)
		homo_file = homo_file + ".csv";

	_string_options["-HOMOGENIZATION_FILE"] = homo_file;
}
