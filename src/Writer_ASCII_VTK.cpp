 ...
 */
Writer_ASCII_VTK::Writer_ASCII_VTK(Problem* prob)
	: Writer_VTK(prob)
{}
Writer_ASCII_VTK::~Writer_ASCII_VTK()
{}
Writer_ASCII_VTK::Writer_ASCII_VTK(const Writer_ASCII_VTK& other)
{
 ...
 */
void Writer_ASCII_VTK::open(std::ofstream& myfile, std::string& filename)
{
 ...
 */
void Writer_ASCII_VTK::write_header(std::ofstream& myfile, double curr_t)
{
 ...
 */
void Writer_ASCII_VTK::write_nodes(std::ofstream& myfile,
 ...
 */
void Writer_ASCII_VTK::write_elements(std::ofstream& myfile,
 ...
 */
void Writer_ASCII_VTK::write_nodal_data(std::ofstream& myfile,
										...
										const std::vector<std::vector<float> >& remote_sol_lists, const std::vector<std::vector<float> >& remote_rhs_lists)
{
	...
	if (mesh->get_rank() == 0)
	{
		...
		// Optionally print out the load information
		if (_printRHS)
		{
 ...
 */
void Writer_ASCII_VTK::write_element_data(std::ofstream& myfile,
			...
			if (_printStrain)
			{
				for (id_type v=0; v<voigt; ++v)
				{
					...
					for (id_type e=0; e<avg_strain.size(); ++e)
						myfile << avg_strain[e][v] << "\n";

					for (id_type part=1; part<n_ranks; ++part)
						for (id_type e=0; e<remote_strain[part-1].size(); ++e)
							myfile << remote_strain[part-1][e][v] << "\n";
				}
			}
			...
			if (_printStress)
			{
				for (id_type v=0; v<voigt; ++v)
				{
					...
					for (id_type e=0; e<avg_stress.size(); ++e)
						myfile << avg_stress[e][v] << "\n";

					for (id_type part=1; part<n_ranks; ++part)
						for (id_type e=0; e<remote_stress[part-1].size(); ++e)
							myfile << remote_stress[part-1][e][v] << "\n";
				}
				...
				myfile << "LOOKUP_TABLE default\n";
				for (id_type e=0; e<avg_stress.size(); ++e)
					myfile << Utilities::von_mises(avg_stress[e]) << "\n";

				for (id_type part=1; part<n_ranks; ++part)
					for (id_type e=0; e<remote_stress[part-1].size(); ++e)
						myfile << Utilities::von_mises(remote_stress[part-1][e]) << "\n";
			}
		}
	}
}
