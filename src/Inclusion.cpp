...
std::vector<double> Inclusion::find_intersection(std::vector<Node>& nodes, double& alpha)
{
	if(nodes.size() != 2)
	...
	if((int_f(0.0, &params)*int_f(1.0, &params)) >= 0)
		err_message("No valid intersecion to be found between nodes " << nodes[0].get_id() << " and " << nodes[1].get_id());
	...
	double alpha0 = 0.5; // Initial guess
	alpha = 0.5;
