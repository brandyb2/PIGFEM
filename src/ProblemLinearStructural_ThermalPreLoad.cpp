...
ProblemLinearStructural_ThermalPreLoad::ProblemLinearStructural_ThermalPreLoad()
...
void ProblemLinearStructural_ThermalPreLoad::generate_assembler()
{
	_assembler = new AssemblerSmallStrainStructural_ThermalPreLoad;
	if (_delta_T_set)
		_assembler->set_parameter("DELTA_T", _delta_T);
}

// This function return the number of degrees of freedom present on each node
id_type ProblemLinearStructural_ThermalPreLoad::nndof()
{
	return _mesh->dim();
}

// Returns the problem type enum for this problem
problem_type ProblemLinearStructural_ThermalPreLoad::get_type()
{
	return LINEAR_STRUCTURAL_THERMALPRELOAD;
}

// Set any possible problem specific parameters
void ProblemLinearStructural_ThermalPreLoad::set_parameter(std::string name, double val)
{
...
double ProblemLinearStructural_ThermalPreLoad::get_parameter(std::string name)
{
	std::transform(name.begin(), name.end(), name.begin(), ::toupper); // Capitilize the name
	if (name=="PLANE_STRAIN" || name=="PLANE STRAIN")
	{
...
void ProblemLinearStructural_ThermalPreLoad::fillBmat(DenseMatrix<double>& B, const std::vector<std::vector<double> >& grad_x)
{
...
void ProblemLinearStructural_ThermalPreLoad::solve_problem()
{
	...
	else
	{
		...
		// temporarily replace the boundaryObject in the problem with the traction free one
