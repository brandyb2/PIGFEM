...
void Assembler::attach_problem(Problem* problem)
{
	_prob = problem;
	if (_prob->subscale())
		_currentMacroscopicValue.resize(_prob->getMacroscaleValue().size());
	std::fill(_currentMacroscopicValue.begin(), _currentMacroscopicValue.end(), 0.0);
}
...
PetscErrorCode Assembler::assemble(pmatrix& K, pvector& P_int,
								   ...
								   NodalData* solution, bool assembleFunc, bool assembleJac)
{
	...
	for(Mesh::element_iterator it=mesh->active_elements_begin(), end=mesh->active_elements_end(); it!=end; ++it)
	{
		...
		assemble_elem((*it), K_el, P_el_int, delta_t, elem_sol, update_ISVs[l_elem], assembleFunc, assembleJac);
...
void Assembler::assemble_elem(Elem* el,
							  ...
							  bool assembleFunc, bool assembleJac)
{
	...
	if (!el->is_intersected())
	{
		...
		for(id_type qp=0; qp<nqp; ++qp)
		{
			...
			if (storedBmats())
			{
				KernelVolumetric (K_temp, P_int_temp,
...
PetscErrorCode Assembler::assemble_new_load_step(Vec& Up, pvector& F_ext, double current_time_frac)
{
	...
	DofObject* dofs = _prob->get_dofs();
	Mesh* mesh = _prob->get_mesh();
	_currentTimeFraction = current_time_frac;

	// For problems with a macroscale load, we need to compute the current macroscopic load
	if (_prob->subscale())
	{
		std::vector<double> maxMacroscaleLoad = _prob->getMacroscaleValue();
		_currentMacroscopicValue = Utilities::scale(maxMacroscaleLoad, current_time_frac);
	}
