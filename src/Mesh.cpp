	  ...
	  _nodes_detected(false), _elements_detected(false), _is_cohesive(false)
{
	_argc = argc;
	_argv = argv;
	PetscInitialize(_argc, _argv, (char*)0, help);
}
...
void Mesh::clear()
{
	...
	_mat_name_to_idx.clear();
	_nodesets.clear();
	_sidesets.clear();
	_proc_neighbors.clear();
	_minimum_bounds.clear();
	_maximum_bounds.clear();
	_dim = 0;
	_rank = 0;
	_nranks = 0;
	_n_global_elem = 0;
	_n_global_nodes = 0;
	_n_local_owned_nodes = 0;
	_partitioned = false;
	...
	_intersected_elem.clear();
	_enrich_node_to_edge.clear();
	_enrich_node_to_inclusion.clear();
	_cohesive_mats.clear();
	_cohesive_to_mat_pair.clear();
...
const std::vector<double>& Mesh::origin() const
{
	if (_was_generated)
		return _origin;
	else
		err_message("Origin of a mesh not generated within PIGFEM is not defined.");
}
...
Elem* Mesh::add_elem(elem_type etype, std::vector<id_type>& node_ids, id_type global_id)
{
	...
	if (elem_in_local_mesh(global_id))
		err_message("An element with the id " << global_id << " already exists in the mesh");
...
Node* Mesh::add_node(double x, double y, double z, id_type global_id, int owner)
{
	...
	if (node_in_local_mesh(global_id))
		err_message("A node with the id " << global_id << " already exists in the mesh");
		...
		else
		{
				...
				for (id_type qp=0; qp<nqp; ++qp)
				{
					...
					for(id_type en=0; en<el->n_enrich_nodes(); ++en)
					{
						...
						for(id_type n=0; n<int_el->n_nodes(); ++n)
						{
							...
							{
								double scale_factor = getEnrichementScaleFactorGlobal(int_node_id);
								shape[el->n_nodes()+en] = scale_factor * child_shape[n];
								grad_x[el->n_nodes()+en] = Utilities::scale(child_grad_x[n], scale_factor);
								break;
							}
						}
					}
			...
			for (id_type ce=0; ce<(*it)->n_cohesive_elem(); ++ce)
			{
				...
				for (id_type qp=0; qp<nqp; ++qp)
				{
					...
					for (id_type n=0; n<(coh_el->n_nodes() / 2); ++n)
					{
						id_type e_id = coh_el->get_node(n)->get_id();
						_shape[local_e][local_qp][n] *= getEnrichementScaleFactorGlobal(e_id);
					}
					local_qp++;
				}
			}
...
void Mesh::generate_node_elem()
{
	...
	*/
	...
	for(;j!=end;++j)
	{
		...
		id_type lnode = global_to_local_node(gnode);
...
// Takes the local node number of an enrichment node and returns a vector of global node numbers that make the edge this enrichment node lies on
std::vector<id_type> Mesh::get_enriched_edge_local(id_type idx)
{
	if (idx >= ENRICH_START)
	{
...
double Mesh::getEnrichementScaleFactorLocal(id_type idx)
{
	// id_type id = get_node_local(idx)->get_id();
	// return -4.*pow((getEnrichmentEdgeDistance(id)-0.5), 2) + 1;
	return 1.0;
}

double Mesh::getEnrichementScaleFactorGlobal(id_type id)
{
	// return -4.*pow((getEnrichmentEdgeDistance(id)-0.5), 2) + 1;
	return 1.0;
}
...
void Mesh::reprocess_nodes(std::unordered_map<id_type, Inclusion*>& nodes_to_reprocess)
{
	...
	for(auto it=nodes_to_reprocess.begin(), end=nodes_to_reprocess.end(); it!=end; ++it)
	{
		...
		if (get_node_owner_local(n)== _rank)
		{
			...
			if (times_moved==5)
				err_message("Unable to move node " << id << " off of an inclusion surface");
	...
	for(auto it=n_expected.begin(), end=n_expected.end(); it!=end; ++it)
	{
		...
		for(int n=0; n<(count0/2); ++n)
		{
			...
			id_type node = global_to_local_node(id);
		...
		if (dl[0]<=(*node)(0) && dl[1]<=(*node)(1) && dl[2]<=(*node)(2) && (*node)(0)<=dl[3] && (*node)(1)<=dl[4] && (*node)(2)<=dl[5])
		{
			...
			else if (res == 1)
			{
				...
				if (_node_partition_interface.find(node->get_id()) == _node_partition_interface.end())
				{
					...
					if (times_moved >= 5);
						err_message("Unable to move node " << node->get_id() << " off of an inclusion surface");
...
void Mesh::modify_node_location(Node* node, Inclusion* inc)
{
	...
	for (id_type i=0; i<dim(); ++i)
		vec[i] = rand();
	double mag = sqrt(Utilities::dot(vec, vec));
	vec = Utilities::scale(vec, 1./mag);

	for (id_type i=0; i<dim(); ++i)
		node->set_coord((*node)(i) + move_dist*vec[i], i);
}
...
void Mesh::add_enrichments()
{
	...
	if (_serial)
	{
		std::set<std::pair<std::vector<id_type>, id_type> > cut_edges;
		std::unordered_map<std::vector<id_type>, std::vector<id_type> > parent_to_refined_edge;
		...
		for(id_type e=0; e<_intersected_elem.size(); ++e)
		{
			...
			for(id_type i=0; i<int_edges.size(); ++i)
			{
				...
				std::vector<id_type> edge;
				for (id_type n=0; n<el->n_nodes_on_edge(int_edges[i]); ++n)
					edge.push_back( (*el)(el->edge_nodes_map(int_edges[i], n)).get_id() );
				std::sort(edge.begin(), edge.begin()+2);		// Sort first two nodes sice they will be the corner/vertex nodes
				if (edge.size() > 2)							// then sort rest of quadratic/whatever node
					std::sort(edge.begin()+3, edge.end());


				// // FIXME: ALL OF THIS HANGING NODE STUFF ONLY WORKS FOR LINEAR EDGES
				// bool has_hanging_node = false;
				// std::vector<id_type> hang;
				// for (id_type n=0; n<edge.size(); ++n)
				// 	if (_edge_hanging_nodes.find(edge[n]); == _edge_hanging_nodes.end())
				// 	{
				// 		has_hanging_node = true;
				// 		hang.push_back(n);
				// 	}
				// if (has_hanging_node)
				// {
				// 	// Dont defer if all are hanging nodes this means that the edge belongs only to a refined element?)
				// 	if (hang.size() != edge.size())
				// 	{
				...
				// 			{
				// 				std::vector<id_type> refined_edge = edge;
				// 				edge = neighbors; // Should already be sorted
				...
				cut_edges.insert( std::pair<std::vector<id_type>, id_type>(edge, inc_nums[i]) );
			}
		}
		...
		_enrichment_edge_dist.resize(cut_edges.size());
		id_type count = 0;
		for(auto it=cut_edges.begin(), end=cut_edges.end(); it!=end; ++it)
		{
			const std::vector<id_type>& edge = it->first;
			...
			for (id_type n=0; n<edge.size(); ++n)
				edge_nodes.push_back( *_nodes[ global_to_local_node(edge[n]) ] );
			...
			std::vector<double> coords = _inclusions[inclusion]->find_intersection(edge_nodes, _enrichment_edge_dist[count]);
		...
		if (is_cohesive())
		{
			...
			_enrichment_edge_dist.resize( n_initial * 2 );
			for(id_type n=0; n<n_initial; ++n)
			{
				...
				_enrichment_edge_dist[n+n_initial] = _enrichment_edge_dist[n];
			}
		}
		...
		for(id_type n=0; n<n_initial; ++n)
		{
			std::vector<id_type> edge = _enrich_node_to_edge[n];
	...
	{
		std::set<std::pair<std::vector<id_type>, id_type> > cut_edges;
		std::set<std::pair<std::vector<id_type>, id_type> > cut_edges_on_interface;
		std::unordered_map<std::vector<id_type>, std::vector<int> > edge_int_map; // Maps an edge to what partitions actually have that edge (defined by have at least one element that shares both of the nodes on the edge)
		std::unordered_map<std::vector<id_type>, std::vector<id_type> > parent_to_refined_edge;
		...
		for(id_type e=0; e<_intersected_elem.size(); ++e)
		{
			...
			if (e==0)
				nNodesPerEdge = el->n_nodes_on_edge(int_edges[0]);
			else
			{
				if (nNodesPerEdge != el->n_nodes_on_edge(int_edges[0]))
					err_message("Mismatch in the number of nodes on each edge of an intersected element!");
			}
			...
			for(id_type i=0; i<int_edges.size(); ++i)
			{
				...
				std::pair<std::vector<id_type>, id_type> insertion(edge, inc_nums[i]);
				...
				std::vector<int> common_parts = partitions_in_common( edge );
				...
				else
				{
					...
					std::map<int, std::vector<id_type> > r_elem = remote_elem_in_common( edge );
					if (r_elem.size() != 0)
					{
		...
		_enrichment_edge_dist.resize(cut_edges.size() + cut_edges_on_interface.size());
		id_type count = 0;
		for(auto it=cut_edges.begin(), end=cut_edges.end(); it!=end; ++it)
		{
			const std::vector<id_type>& edge = it->first;
			id_type inclusion = it->second;
			// Form a vector of nodes that define the edge that is intersected. This can later be extended to 3+ nodes
			std::vector<Node> edge_nodes(edge.size());
			for (id_type n=0; n<edge.size(); ++n)
				edge_nodes[n] = *_nodes[ global_to_local_node(edge[n]) ];
		...
		{
			std::vector<id_type> edge = it->first;
			id_type inclusion = it->second;
			...
			int owner = get_node_owner_global(edge[0]);
		...
		for(id_type n=n_local_interior_e_nodes; n<_n_local_owned_enrich_nodes; ++n)
		{
			...
			for(id_type p=0; p<parts.size(); ++p)
			{
				...
				if (part != _rank) // Only need to do this for ranks that are not my own
				{
					...
					for (id_type n2=0; n2<edge.size(); ++n2)
						id_send_lists[part].push_back( edge[n2] );
					...
					coord_send_lists[part].push_back( getEnrichmentEdgeDistance(e_node->get_id()) );
		...
		int msg_count = nNodesPerEdge + ids_per_coords + 1; // 2 for the 2 edge nodes, 1 for the inclusion number
		for(auto it=n_expected.begin(), end=n_expected.end(); it!=end; ++it)
		{
			int part = (*it).first;
			...
			{
				std::vector<id_type> edge;
				for (id_type n2=0; n2<nNodesPerEdge; ++n2)
					edge.push_back( id_recv_vec[n*msg_count+n2] );

				id_type e_id = id_recv_vec[n*msg_count + nNodesPerEdge];
				id_type mirror_id = 0;
				id_type inclusion = 0;
				if (is_cohesive())
				{
					mirror_id = id_recv_vec[n*msg_count + nNodesPerEdge + 1];
					inclusion = id_recv_vec[n*msg_count + nNodesPerEdge + 2];
				}
				else
				{
					inclusion = id_recv_vec[n*msg_count + nNodesPerEdge + 1];
				}

				std::pair<std::vector<id_type>, id_type> edge_inc(edge, inclusion);
				...
				double x = coord_recv_vec[n*4];
				double y = coord_recv_vec[n*4+1];
				double z = coord_recv_vec[n*4+2];
				double alpha = coord_recv_vec[n*4+3];
				_global_to_local_enrich_node[e_id] = count + ENRICH_START;
				_enrich_nodes[count] = new Node(x, y, z, e_id);
				_enrich_owners[count] = part;
				_enrich_node_to_edge[count] = edge;
				_enrich_node_to_inclusion[count] = inclusion;
				_enrichment_edge_dist[count] = alpha;
		...
		if (is_cohesive())
		{
			...
			_enrichment_edge_dist.resize(2*n_initial);
			std::copy_n(_enrichment_edge_dist.begin(), n_initial, _enrichment_edge_dist.begin()+n_initial);
...
void Mesh::add_enrich_node_to_elem(std::vector<id_type> edge, id_type inclusion, id_type n, id_type n_initial)
{
	std::vector<id_type> common = local_elem_in_common(edge);
	...
	for(id_type e=0; e<common.size(); ++e)
	{
		...
		int correct_edge = el->get_edge_from_nodes(edge);
		if (correct_edge == -1)
...
// FIXME: This only works for linear edges
double Mesh::getEnrichmentEdgeDistance(id_type id)
{
	id_type idx = global_to_local_node(id);
	if (idx >= ENRICH_START)
	{
		if ((idx-ENRICH_START) < n_local_enrich_nodes())
			return _enrichment_edge_dist[idx-ENRICH_START];
		else
			err_message("Attempting to get the enrichment edge distance of an ode that does not exist on the local mesh");
	}
	else
		err_message("Attempting to get the enrichment edge distance of a non-enrichment node: id=" << id);
}


std::vector<id_type> Mesh::get_edge_from_enrich_node(id_type enrich_id)
{
...
void Mesh::set_constraint_values()
{
	...
	for (auto it=_edge_hanging_nodes.begin(), end=_edge_hanging_nodes.end(); it!=end; ++it)
	{
		...
		if (it2 != edge_hanging_to_enrich_nodes.end())
		{
			...
			double interpolate = getEnrichmentEdgeDistance( enrich_id );
...
void Mesh::find_hanging_enriched_neighbors(std::unordered_map<id_type, id_type>& edges_map, std::unordered_map<id_type, std::vector<id_type> >& faces_map)
{
	...
	std::unordered_map<std::vector<id_type>, id_type> edge_to_enrich_node;
	...
	for (id_type n=0; n<nn; ++n)
		edge_to_enrich_node.insert( std::pair<std::vector<id_type>, id_type>(_enrich_node_to_edge[n], n) );
	...
	for (auto it=_edge_hanging_nodes.begin(), end=_edge_hanging_nodes.end(); it!=end; ++it)
	{
		...
		if (neighbors.size() == 2)
		{
			auto it2 = edge_to_enrich_node.find(neighbors);
			if (it2 != edge_to_enrich_node.end())
			{
				...
				else
				{
					...
					double interpolate = getEnrichmentEdgeDistance( enrich_id ); // See how far along the edge the enrichment node is
