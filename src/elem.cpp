...
Elem::~Elem()
{
	// Since the integration elements are only stored here they need to be deleted here
	if (n_integration_elem() != 1) // Case where I only reference myself
	{
								 ...
								 std::vector<double>& v, double& div_v)
{
	...
	// 	for (id_type d=0; d<dim(); ++d)
	// 		div_v += dNdx[n+n_nodes()][d] * nodal_velocity[(n+n_nodes())*dim() + d];
...
id_type Elem::n_integration_elem()
{
	if (_is_intersected)
		return _integrationElem.size();
	else
		return 1;
}
...
/*
 * Default element detection is done assuming cohesive behavior (C-1)
 * If this is not the case, this function converts the structures
 * back to the normal (C0) structures
*/
void Elem::transformDetectionCohToReg()
{
	int n_enrich = _cutEdge.size();
	for (id_type el=0; el<_int_elem_struct.size(); ++el)
		for (id_type node=0; node<_int_elem_struct[el].size(); ++node)
			if (_int_elem_struct[el][node] >= (n_nodes()+n_enrich))
				_int_elem_struct[el][node] -= n_enrich; // Transform the mirror nodes back to the normal enrichment nodes
