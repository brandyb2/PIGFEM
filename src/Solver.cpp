...
PetscErrorCode Solver::preallocate_vectors()
{
	PetscErrorCode ierr;
...
void Solver::Output(id_type step_iter, double current_t)
{
	...
	if (_prob->getOptions()->getBoolOption("-HOMOGENIZE"))
	{
		std::string filename = out_folder + _prob->getOptions()->getOption("-HOMOGENIZATION_FILE");
		_prob->homogenize();
		_prob->getHomogenizationWriter()->writeConsecutive(filename, current_t, step_iter);
	}
