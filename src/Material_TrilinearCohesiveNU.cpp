...
Material* TrilinearCohesiveNUMaterial::allocate()
{
	return new TrilinearCohesiveNUMaterial;
}
// This function assumes the current material is set up correctly. Which it should't be possible for it not to be
void TrilinearCohesiveNUMaterial::copy(Material* other_mat)
{
...
TrilinearCohesiveNUMaterial::TrilinearCohesiveNUMaterial()
...
double TrilinearCohesiveNUMaterial::getDeltaEff(std::vector<double>& delta_ttn)
{
...
Material::output_params* TrilinearCohesiveNUMaterial::Constitutive(Material::input_params& input)
{
	if(!ready)
	...
	{
		trac_eff = _sigma_c * (delta_eff / _delta_c1);
	...
	{
		trac_eff = _sigma_c * (1 - (delta_eff-_delta_c2) / (_delta_c3 - _delta_c2) );
		Dtraction_Ddelta = -1.0 * (_sigma_c / (_delta_c3 - _delta_c2));
	}
	else
	{
		trac_eff = 0;
...
Material::sensitivity_output_params* TrilinearCohesiveNUMaterial::SensitivityConstitutive(Material::sensitivity_input_params& input)
{
...
SensitivityMaterialParameter* TrilinearCohesiveNUMaterial::getSensitivityParameter(std::string param_name)
{
	err_message("Sensitivity is not implemented for the bilinear cohesive model");
...
double TrilinearCohesiveNUMaterial::getNormalizedCohesiveFailure(std::vector<double>& delta_ttn)
{
	double delta_eff = getDeltaEff(delta_ttn);
	return (delta_eff / _delta_c1);
}
id_type TrilinearCohesiveNUMaterial::get_cohesive_damage_zone(std::vector<double>& delta)
{
...
void TrilinearCohesiveNUMaterial::set_parameter(std::string name, double val)
{
...
double TrilinearCohesiveNUMaterial::get_parameter(std::string name)
{
	std::transform(name.begin(), name.end(), name.begin(), ::toupper); // Capitilize the name
	if (name=="SIGMA" || name=="SIGMA_C" || name=="SIGMA C" || name=="SIGMA_CRITICAL" || name=="SIGMA CRITICAL")
		return get_sc();
...
void TrilinearCohesiveNUMaterial::set_dc1(double dc)
{
...
double TrilinearCohesiveNUMaterial::get_dc1()
{
	if (_d2_set)
		return _delta_c2;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return _delta_c2 + _delta_c2 - 2.0*_G/_sigma_c;
	else
...
void TrilinearCohesiveNUMaterial::set_dc2(double dc)
{
...
double TrilinearCohesiveNUMaterial::get_dc2()
{
	if (_d2_set)
		return _delta_c2;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return 2.0*_G/_sigma_c + _delta_c1 - _delta_c3;
	else
...
void TrilinearCohesiveNUMaterial::set_dc3(double dc)
{
...
double TrilinearCohesiveNUMaterial::get_dc3()
{
	if (_d3_set)
		return _delta_c3;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return 2.0*_G/_sigma_c + _delta_c1 - _delta_c2;
	else
...
void TrilinearCohesiveNUMaterial::set_sc(double sc)
{
...
double TrilinearCohesiveNUMaterial::get_sc()
{
	if (_s_set)
		return _sigma_c;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return 2.0 * _G / (_delta_c2 - _delta_c1 + _delta_c3);
	else
...
void TrilinearCohesiveNUMaterial::set_G(double G)
{
...
double TrilinearCohesiveNUMaterial::get_G()
{
	if (_G_set)
		return _G;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return 0.5 * _sigma_c * (_delta_c2 - _delta_c1 + _delta_c3);
	else
...
void TrilinearCohesiveNUMaterial::set_b(double b)
{
	if (b > 0)
		_beta2 = b*b;
	else
...
void TrilinearCohesiveNUMaterial::set_alpha(double a)
{
	if (a > 0)
		_alpha = a;
	else
