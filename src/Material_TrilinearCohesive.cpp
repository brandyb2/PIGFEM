...
Material* TrilinearCohesiveMaterial::allocate()
{
	return new TrilinearCohesiveMaterial;
}


std::vector<std::string> TrilinearCohesiveMaterial::internal_vars_name()
{
	std::vector<std::string> vec = {"maximum_opening"};
	return vec;
}

std::vector<bool> TrilinearCohesiveMaterial::internal_vars_print()
{
...
Material::output_params* TrilinearCohesiveMaterial::Constitutive(Material::input_params& input)
{
	if(!ready)
	...
	else	// Unloading. Unload to origin and don't update ISV
	{
		if (delta_max >= 0 && delta_max < _delta_c1)
			...
			max_traction = _sigma_c * (1 - (delta_max-_delta_c2) / (_delta_c3 - _delta_c2) );
		else
			max_traction = 0;
...
Material::sensitivity_output_params* TrilinearCohesiveMaterial::SensitivityConstitutive(Material::sensitivity_input_params& input)
{
...
void TrilinearCohesiveMaterial::updateSensitivityISVs(Material::sensitivity_input_params& input)
{
	err_message("Sensitivity is not implemented for the bilinear cohesive model");
	// if(!ready)
	// 	err_message("Not all material parameters have been set for the Continuum Damage Material.");
