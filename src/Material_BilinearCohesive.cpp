...
Material* BilinearCohesiveMaterial::allocate()
{
	return new BilinearCohesiveMaterial;
}


std::vector<std::string> BilinearCohesiveMaterial::internal_vars_name()
{
	std::vector<std::string> vec = {"maximum_opening"};
	return vec;
}

std::vector<bool> BilinearCohesiveMaterial::internal_vars_print()
{
...
Material::output_params* BilinearCohesiveMaterial::Constitutive(Material::input_params& input)
{
	if(!ready)
		err_message("Not all material parameters have been set for the Bilinear Cohesive material.");

	// Get the internal variables and references to the output
	if((*input.internal_vars).size() != 1)
		err_message("Invalid number of internal variables for a Bilinear Cohesive material.");
	...
	else	// Unloading. Unload to origin and don't update ISV
	{
		...
		if (delta_max >= 0 && delta_max < _delta_c1)
			max_traction = delta_max * (_sigma_c / _delta_c1);
		else if (delta_max >= _delta_c1 && delta_max<_delta_c2)
			max_traction = _sigma_c * (1 - (delta_max-_delta_c1) / (_delta_c2 - _delta_c1) );
		else
			max_traction = 0;
...
Material::sensitivity_output_params* BilinearCohesiveMaterial::SensitivityConstitutive(Material::sensitivity_input_params& input)
{
...
void BilinearCohesiveMaterial::updateSensitivityISVs(Material::sensitivity_input_params& input)
{
	err_message("Sensitivity is not implemented for the bilinear cohesive model");
	// if(!ready)
	// 	err_message("Not all material parameters have been set for the Continuum Damage Material.");
