	...
	for(Mesh::element_iterator it=mesh->active_elements_begin(), end=mesh->active_elements_end(); it!=end; ++it)
	{
		...
		if((*it)->is_intersected())
		{
			...
			for(id_type ie=0; ie<n_int_elem; ++ie)
			{
				...
				// Compute the integration elem volumes
				Integrator<double> volumeIntegrator;
				_int_elem_vols[g_elem][ie] = volumeIntegrator.integrateElemSimple(_prob, el, &volume, ie);
			...
			_elem_vols[l_elem] = std::accumulate(_int_elem_vols[g_elem].begin(), _int_elem_vols[g_elem].end(), 0.0);
		...
		else
		{
			...
			Integrator<double> volumeIntegrator;
			_elem_vols[g_elem] = volumeIntegrator.integrateElemSimple(_prob, el, &volume);
...
void Writer_VTK_Nodal::writeFromStream(std::ofstream& myfile, double curr_t)
{
	...
	// Gather and write data associated with the nodes (stress, strain, solution, external load)
	write_node_info(myfile);

	// Gather and write data associated with the elements (internal variables, material)
...
/*
 * Gather and write data associated with the elements (nternal variables, material)
 */
	...
	for(Mesh::node_iterator it=mesh->nodes_begin(), end=mesh->nodes_end(); it!=end; ++it)
	{
		...
		if(mesh->own_node_local(l_node))
		{
			...
			if (_prob->subscale())
			{
				...
				std::vector<double> macro_disp = _prob->computeMacroscaleSolutionContribution(coords);
				for(id_type d=0; d<nndof; ++d)
															  ...
															  std::vector<std::vector<float> >& node_volume_contributions)
{
	...
	fillLocalElementalStressStrains(elem_stresses, elem_strains);
...
void Writer_VTK_Nodal::fillLocalElementalStressStrains(std::vector<std::vector<std::vector<double> > >& elem_strains,
													   std::vector<std::vector<std::vector<double> > >& elem_stresses)
{
	Mesh* mesh = _prob->get_mesh();
	elem_strains.resize(mesh->n_local_elem());
	elem_stresses.resize(mesh->n_local_elem());
	myFunctor<std::vector<double> >* strainFunctor = _prob->getStrainKernel();
	strainFunctor->_ptr = (void*) _prob;
	myFunctor<std::vector<double> >* stressFunctor = _prob->getStrainKernel();
	stressFunctor->_ptr = (void*) _prob;

	// Loop over all elements and compute all stresses and strains at every gauss point
	for (Mesh::element_iterator it=mesh->active_elements_begin(), end=mesh->active_elements_end(); it!=end; ++it)
	{
		Elem* el = (*it);

		id_type l_elem = mesh->global_to_local_elem(el->get_id());
		id_type nvqp = mesh->n_volumetric_qps(l_elem);
		elem_strains[l_elem].resize(nvqp);
		elem_stresses[l_elem].resize(nvqp);

		// Assemble the current solution vector
		DenseMatrix<double> elem_sol(0);
		ProblemUtilities::formElementalMatrix(elem_sol, el, _prob->get_solution());
		...
		{
			Elem* int_el = el->get_integration_elem(ie);
			Material* curr_mat = mesh->get_element_material_local(el->get_id(), ie);
			...
			for(id_type qp=0; qp<nqp; ++qp)
			{
				...
				currentState.ISVs = &_prob->get_internal_vars()->get_internal_vars_local(l_elem, curr_qp);

				// Compute the output
				elem_strains[l_elem][curr_qp] = strainFunctor->evaluate(&currentState);
				elem_stresses[l_elem][curr_qp] = stressFunctor->evaluate(&currentState);
	...
	delete strainFunctor;
	delete stressFunctor;
}
