...
void WriterPath::writeFromStream(std::ofstream& myfile, double curr_t)
{
	...
	myFunctor<std::vector<double> >* stressFunctor = _prob->getStressKernel();
	for (id_type p=0; p<_points.size(); ++p)
	{
		if (_on_partition[p])
		{
			// Assemble the current solution vector
			Elem* el = mesh->get_elem_global(_elements[p]);
			...
			if (_output_stress)
			{
				...
				currentState.Mat = _mats[p];
				currentState.dN = &_dN[p];
				// id_type l_elem = mesh->global_to_local_elem(el->get_id());
				// currentState.ISVs = &_prob->get_internal_vars()->get_internal_vars_local(l_elem, curr_qp); // How to do ISVs with a path output? Projection from QPs?

				stress[p] = stressFunctor->evaluate(&currentState);
			}
		} // End loop over neighboring elements
	} // end points loop
