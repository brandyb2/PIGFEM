...
Material* BilinearCohesiveNUMaterial::allocate()
{
	return new BilinearCohesiveNUMaterial;
}
// This function assumes the current material is set up correctly. Which it should't be possible for it not to be
void BilinearCohesiveNUMaterial::copy(Material* other_mat)
{
...
BilinearCohesiveNUMaterial::BilinearCohesiveNUMaterial()
	: _sigma_c(0.0), _delta_c1(0.0), _delta_c2(0.0), _G(0.0), _beta2(1.0), _alpha(50.0),
	  _s_set(false), _d1_set(false), _d2_set(false), _G_set(false), ready(false)
{
}

double BilinearCohesiveNUMaterial::getDeltaEff(std::vector<double>& delta_ttn)
{
...
Material::output_params* BilinearCohesiveNUMaterial::Constitutive(Material::input_params& input)
{
	if(!ready)
		err_message("Not all material parameters have been set for the Bilinear No Unloading Cohesive material.");
	...
	{
		trac_eff = delta_eff * (_sigma_c / _delta_c1);
		Dtraction_Ddelta = (_sigma_c / _delta_c1);
	}
	else if (delta_eff >= _delta_c1 && delta_eff<_delta_c2)
	{
		trac_eff = _sigma_c * (1 - (delta_eff-_delta_c1) / (_delta_c2 - _delta_c1) );
		Dtraction_Ddelta = -1.0 * (_sigma_c / (_delta_c2 - _delta_c1));
	}
	else
	{
		trac_eff = 0;
	...
	{
		traction_ttn[dim-1] = (_sigma_c / _alpha) * sinh(_alpha * delta_ttn[dim-1] / _delta_c1);
		for (id_type i=0; i<(dim-1); ++i)
			Dmat(dim-1, i) = 0.0;
		Dmat(dim-1, dim-1) = (_sigma_c / _delta_c1) * cosh(_alpha * delta_ttn[dim-1] / _delta_c1);
	}
...
Material::sensitivity_output_params* BilinearCohesiveNUMaterial::SensitivityConstitutive(Material::sensitivity_input_params& input)
{
...
SensitivityMaterialParameter* BilinearCohesiveNUMaterial::getSensitivityParameter(std::string param_name)
{
	err_message("Sensitivity is not implemented for the bilinear cohesive model");
...
double BilinearCohesiveNUMaterial::getNormalizedCohesiveFailure(std::vector<double>& delta_ttn)
{
	double delta_eff = getDeltaEff(delta_ttn);
	return (delta_eff / _delta_c1);
}
id_type BilinearCohesiveNUMaterial::get_cohesive_damage_zone(std::vector<double>& delta)
{
...
void BilinearCohesiveNUMaterial::set_parameter(std::string name, double val)
{
	...
	if (name=="SIGMA" || name=="SIGMA_C" || name=="SIGMA C" || name=="SIGMA_CRITICAL" || name=="SIGMA CRITICAL")
		...
		set_dc1(val);
	else if (name=="DELTA2" || name=="DELTA_C2" || name=="DELTA C2" || name=="DELTA_FINAL" || name=="DELTA FINAL")
		set_dc2(val);
...
double BilinearCohesiveNUMaterial::get_parameter(std::string name)
{
	std::transform(name.begin(), name.end(), name.begin(), ::toupper); // Capitilize the name
	if (name=="SIGMA" || name=="SIGMA_C" || name=="SIGMA C" || name=="SIGMA_CRITICAL" || name=="SIGMA CRITICAL")
		return get_sc();
...
void BilinearCohesiveNUMaterial::set_dc1(double dc)
{
	_delta_c1 = dc;
	_d1_set = true;

	if (_d2_set)
		if (_delta_c2 < _delta_c1)
			err_message("Incompatible parameter delta_c1 added to bilinear cohesive law");

	if (_d2_set && _s_set){
		ready = true;
		_G = get_G();
	}
	else if (_d2_set && _G_set){
		ready = true;
		_sigma_c = get_sc();
	}
	else if (_G_set && _s_set){
		ready = true;
		_delta_c2 = get_dc2();
	}
}
double BilinearCohesiveNUMaterial::get_dc1()
{
	if(_d1_set)
		return _delta_c1;
	else
...
void BilinearCohesiveNUMaterial::set_dc2(double dc)
{
	if (_s_set && _G_set)
		err_message("Can only specify 2 parameters out of G, sigma_c, and delta_c2");
	else{
		_delta_c2 = dc;
		_d2_set = true;
		...
		if (_d1_set)
			if (_delta_c2 < _delta_c1)
				err_message("Incompatible parameter delta_c2 added to bilinear cohesive law");

		if (_d1_set && _s_set){
			ready = true;
			_G = get_G();
		}
		else if (_d1_set && _G_set){
			ready = true;
			_sigma_c = get_sc();
		}
	}	
}
double BilinearCohesiveNUMaterial::get_dc2()
{
	if (_d2_set)
		return _delta_c2;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return 2.0 * _G / _sigma_c;
	else
...
void BilinearCohesiveNUMaterial::set_sc(double sc)
{
	if (_d2_set && _G_set)
		err_message("Can only specify 2 parameters out of G, sigma_c, and delta_c2");
	else{
		...
		if (_sigma_c < 0.0)
			err_message("Incompatible parameter sigma_c added to bilinear cohesive law");
...
double BilinearCohesiveNUMaterial::get_sc()
{
	if (_s_set)
		return _sigma_c;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
		return 2.0 * _G / _delta_c2;
	else
...
void BilinearCohesiveNUMaterial::set_G(double G)
{
	if (_d2_set && _s_set)
		err_message("Can only specify 2 parameters out of G, sigma_c, and delta_c2");
	else{
		...
		if (_d1_set && _s_set)
			if (_G < (0.5 * _sigma_c * _delta_c1))
				err_message("Incompatible parameter G added to bilinear cohesive law");

		if (_d1_set && _d2_set){
			ready = true;
			_sigma_c = get_sc();
		}
		else if (_d1_set && _s_set){
			ready = true;
			_delta_c2 = get_dc2();
		}
	}	
}
double BilinearCohesiveNUMaterial::get_G()
{
	if (_s_set)
		return _sigma_c;
	else if (ready)     // The other 3 parameters are specified. Compute this one from them{
...
void BilinearCohesiveNUMaterial::set_b(double b)
{
	if (b > 0)
		_beta2 = b*b;
	else
...
void BilinearCohesiveNUMaterial::set_alpha(double a)
{
	if (a > 0)
		_alpha = a;
	else
