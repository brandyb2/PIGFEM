...
// Same thing as above but with the solution in matrix form
void ProblemUtilities::SmallStrainFast(std::vector<double>& strain, const std::vector<std::vector<double> >& dN, const DenseMatrix<double>& U)
{
	unsigned char n = dN.size();
	unsigned char dim = dN[0].size(); // Assumes there are nodes
	...
	switch (dim)
	{
		case 2: // Most common case probably
			for (id_type node=0; node<n; ++node)
				strain[0] += dN[node][0] * U(node, 0);
			for (id_type node=0; node<n; ++node)
				strain[1] += dN[node][1] * U(node, 1);
			for (id_type node=0; node<n; ++node)
				strain[2] += dN[node][1]*U(node, 0) + dN[node][0]*U(node, 1);
			break;
		...
		case 3:
			...
			for (id_type node=0; node<n; ++node)
				strain[1] += dN[node][1] * U(node, 1);
			for (id_type node=0; node<n; ++node)
				strain[2] += dN[node][2] * U(node, 2);
			for (id_type node=0; node<n; ++node)
				strain[3] += dN[node][2]*U(node, 1) + dN[node][1]*U(node, 2);
			for (id_type node=0; node<n; ++node)
				strain[4] += dN[node][2]*U(node, 0) + dN[node][0]*U(node, 2);
			for (id_type node=0; node<n; ++node)
				strain[5] += dN[node][1]*U(node, 0) + dN[node][0]*U(node, 1);
			break;
...
void ProblemUtilities::Bt_D_B(DenseMatrix<double>& K, const DenseMatrix<double>& B, const DenseMatrix<double>& D)
{
	int in = D.n_rows();
	int ex = B.n_cols();
...
void ProblemUtilities::B_U(std::vector<double>& strain, const DenseMatrix<double>& B, const std::vector<double>& U)
{
...
void ProblemUtilities::Bt_F(std::vector<double>& P, const DenseMatrix<double>& B, const std::vector<double>& sigma)
{
	P.resize(B.n_cols());
...
// This function forms the elemental solution vector
void ProblemUtilities::formElementalMatrix(DenseMatrix<double>& elem_mat, Elem* el, NodalData* data)
{
	// Assemble the current displacement vector
	id_type nn = el->n_nodes() + el->n_enrich_nodes();
	id_type nndof = data->nndof();
	elem_mat.resize(nn, nndof);
	for(id_type n=0; n<nn; ++n)
	{
		id_type g_node;
		if (n < el->n_nodes())
		...
		for(id_type d=0; d<nndof; ++d)
			elem_mat(n, d) = data->get_value_global(g_node, d);
	}
}
...
std::vector<float> Utilities::scale(const std::vector<float>& vec, float alpha)
{
...
int Utilities::isBigEndian(void)
{
    union {
        uint32_t i;
        char c[4];
