...
{
	_assembler = new AssemblerSmallStrainStructural;
}
...
{
	VolumeFunctor vol;
	myFunctor<std::vector<double> > *stressFunctor = getStressKernel();
	Integrator<double> volumeIntegrator;
	Integrator<std::vector<double> > stressIntegrator;
	...
	for (Mesh::element_iterator it=_mesh->active_elements_begin(), end=_mesh->active_elements_end(); it!=end; ++it)
	{
		...
		for (id_type ie=0; ie<el->n_integration_elem(); ++ie)
		{
			...
			else
			{
				...
				{
					std::vector<double> stressVol = stressIntegrator.integrateElem(this, el, stressFunctor, ie);
					double Volume = volumeIntegrator.integrateElemSimple(this, el, &vol, ie);
					std::vector<double> stress = Utilities::scale(stressVol, 1./Volume);
					std::vector<double> principal = Utilities::principal_stress_strain(stress);
