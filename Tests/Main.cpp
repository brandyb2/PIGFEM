...
#include "Material_BilinearCohesive.h"
#include "Material_BilinearCohesiveNU.h"
#include "Material_TrilinearCohesive.h"
#include "Material_TrilinearCohesiveNU.h"
...
#define yexpansion 0.005
#define zexpansion 0.005
#define yexpansion_plies 0.10	// Increase the size of the domain (add matrix) by this much on each side of the domain (0.05=5%)
#define bounding_ply_limit 0.09 // the transversely isotropic bounding plies cover top and bottom 17% of the domain

// Set some flags here
#define cohesive_flag true
#define max_principal_stress_flag false
#define continuum_damage_flag false
#define bounding_plies_flag true
...
void read_stl(Mesh* mesh, std::string filename, std::vector<double>& maxes, std::vector<double>& mins, double& min_rad, int& dimension,
			  std::vector<double>& sigma_c, std::vector<double>& delta_c, bool& individual);
std::string getParameter(Options* options, std::vector<std::string> param_names, bool& set);
double getDoubleParameter(Options* options, std::vector<std::string> param_names, bool& set);
...
int main (int argc, char* argv[])
{
	...
	{
		if (linear)
			problem = new ProblemLinearStructural_ThermalPreLoad;
		else
			problem = new ProblemNonlinearStructural_ThermalPreLoad;
	...
	// Assign materials
	{
		...
		{
			Material* coh_mat;
			bool coh_choice_set = false;
			std::vector<std::string> params = {"-COH_MODEL"};
			std::string coh_choice = getParameter(problem->getOptions(), params, coh_choice_set);
			std::transform(coh_choice.begin(), coh_choice.end(), coh_choice.begin(), ::toupper);

			if (coh_choice == "OP" || coh_choice == "ORTIZ-PANDOLFI" || coh_choice == "") // Default
				coh_mat = new OPCohesiveAugMaterial;
			else if (coh_choice == "XN" || coh_choice == "XU-NEEDLEMAN")
				coh_mat = new XNCohesiveMaterial;
			else if (coh_choice == "BL" || coh_choice == "BILINEAR")
				coh_mat = new BilinearCohesiveMaterial;
			else if (coh_choice == "TL" || coh_choice == "TRILINEAR")
				coh_mat = new TrilinearSmoothedCohesiveMaterial;
				// coh_mat = new TrilinearCohesiveMaterial;
			else if (coh_choice == "OPNU" || coh_choice == "ORTIZ-PANDOLFI-NU")
				coh_mat = new OPCohesiveNUAugMaterial;
			else if (coh_choice == "XNNU" || coh_choice == "XU-NEEDLEMAN-NU")
				coh_mat = new XNCohesiveNUMaterial;
			else if (coh_choice == "BLNU" || coh_choice == "BILINEAR-NU")
				coh_mat = new BilinearCohesiveNUMaterial;
			else if (coh_choice == "TLNU" || coh_choice == "TRILINEAR-NU")
				coh_mat = new TrilinearSmoothedCohesiveMaterial;
				// coh_mat = new TrilinearCohesiveNUMaterial;
			else
			...
			if (!individual)
			{
				...
				// Get the peak traction
				bool sig_set;
				params = {"-SIGMA_C", "-SC"};
				double sig_c = getDoubleParameter(problem->getOptions(), params, sig_set);
				if (!sig_set)
					sig_c = sigmac;

				bool del_set = false;
				if (coh_mat->get_type() == BL_COHESIVE || coh_mat->get_type() == BL_COHESIVE_NO_UNLOADING){
					coh_mat->set_parameter("SIGMA_C", sig_c);

					params = {"-DELTA_C", "-DC"};
					double default_delc = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						default_delc = deltac;

					// Set the second of the 2 delta_c values
					params = {"-DELTA_C1", "-DC1"};
					double del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = default_delc;
					coh_mat->set_parameter("DELTA_C1", del_c);

					// Set the second of the 2 delta_c values
					params = {"-DELTA_C2", "-DC2"};
					del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = 4*default_delc; // default to 4 times the default value
					coh_mat->set_parameter("DELTA_C2", del_c);
				}
				else if (coh_mat->get_type() == TL_COHESIVE || coh_mat->get_type() == TL_COHESIVE_NO_UNLOADING){
					coh_mat->set_parameter("SIGMA_C", sig_c);
					...
					// Set the first of the 3 delta_c values
					params = {"-DELTA_C1", "-DC1"};
					double del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = default_delc;
					...
					// Set the second of the 3 delta_c values
					params = {"-DELTA_C2", "-DC2"};
					del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = 2*default_delc;
					coh_mat->set_parameter("DELTA_C2", del_c);

					// Set the third of the 3 delta_c values
					params = {"-DELTA_C3", "-DC3"};
					del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = 4*default_delc;
					coh_mat->set_parameter("DELTA_C3", del_c);
				}
				else if (coh_mat->get_type() == OP_COHESIVE_AUGMENTED || coh_mat->get_type() == OP_COHESIVE_NO_UNLOADING_AUGMENTED){
					coh_mat->set_parameter("SIGMA_C", sig_c);

					// Set the delta_c value
					params = {"-DELTA_C", "-DC"};
					double del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = deltac;
					coh_mat->set_parameter("DELTA_C", del_c);
					...
					// Set the normal critical opening
					params = {"-DELTA_CN", "-DCN"};
					double del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = default_delc;
					coh_mat->set_parameter("DELTA_CN", del_c);

					// Set the tangential critical opening
					params = {"-DELTA_CT", "-DCT"};
					del_c = getDoubleParameter(problem->getOptions(), params, del_set);
					if (!del_set)
						del_c = default_delc;
					coh_mat->set_parameter("DELTA_CT", del_c);
			...
			else
			{
				...
				for (id_type m=0; m<sigma_c.size(); ++m)
				{
					...
					if (coh_mat->get_type() == BL_COHESIVE || coh_mat->get_type() == BL_COHESIVE_NO_UNLOADING){
						...
						coh_mat->set_parameter("DELTA_C1", delta_c[m]);
						coh_mat->set_parameter("DELTA_C2", 4*delta_c[m]);
					}
					else if (coh_mat->get_type() == TL_COHESIVE || coh_mat->get_type() == TL_COHESIVE_NO_UNLOADING){
						coh_mat->set_parameter("SIGMA_C", sigma_c[m]);
						coh_mat->set_parameter("DELTA_C1", delta_c[m]);
						coh_mat->set_parameter("DELTA_C2", 2*delta_c[m]);
						coh_mat->set_parameter("DELTA_C3", 4*delta_c[m]);
					}
					else if (coh_mat->get_type() == OP_COHESIVE_AUGMENTED || coh_mat->get_type() == OP_COHESIVE_NO_UNLOADING_AUGMENTED){
						coh_mat->set_parameter("SIGMA_C", sigma_c[m]);
	...
	// Assign BCs (FIXME: DOESN'T MAKE A GOOD SYSTEM IN 3D)
	{
		if (subscale)
		{
			...
			problem->setMacroscaleValue(strain_load);

			// Assign zero perturbation displacement at the corners
			// Does this keep the homogenized perturbation strain zero?
...
{
	set = false;
	for (id_type i=0; i<param_names.size(); ++i)
	{
		if (options->optionSet(param_names[i])){
			set = true;
			return options->getOption(param_names[i]);
		}
		else
			continue;
	}
...
double getDoubleParameter(Options* options, std::vector<std::string> param_names, bool& set)
{
	...
	for (id_type i=0; i<param_names.size(); ++i)
	{
		if (options->optionSet(param_names[i])){
			set = true;
			return std::stod( options->getOption(param_names[i]) );
		}
		else
			continue;
	}
